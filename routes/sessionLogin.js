...
router.post('/:alumnoId/fotoPerfil', upload.single('foto'), async (req, res) => {
  ...
  const params = {
    ...
    ACL: 'public-read' // Asegurar que el archivo sea público
  };
  ...
  try {
    ...
    const data = await AWS.send(new PutObjectCommand(params));  // Usamos PutObjectCommand
